...
sim |> 
  ...
  scale_fill_manual(values=c("#f1b6da", "#7fbf7b"))+
  theme_minimal()

# One 1 row with N/A value for Distance is removed

sim |> summarize(M = mean(Distance, na.rm = TRUE),
...
# Warning message tells us that the aforementioned N/A row was removed, ignore warning.
```
...
plot_normal(mean = 78, sd = 40) +
  xlim(3,214)

# The means are taken from the descriptive section.
# The xlim are based on the fact that the data points in Distance range from 3 to 214. 
...
# The priors for Intercept (Avg distance for Sem = Different) are taken from the
# mean. The priors for class = b are for the slope, i.e. the difference from
# SemDifferent to SemSimilar. I assume -10 based on the part (A). 
```
...
# Seems good. The prior predictive check basically says that the distance between
# SemSimilar is likely to be a tad lower than SemDifferent.
```
...
# The full model give indeed much more narrower and shifted posteriors, with
# the distance between SemSimilar clearly around 10 units lower than SemDifferent.
...
# This seems cool, I interpret it to mean that regarding the change from
# SemDifferent to SemSimilar, first of all, the chance to decrease in distance is 
# a lot bigger than increase. Furthermore, the likeliest change seems to hover 
# around -4. 
```
...
# The data and the posterior samples don't match up too well, since the posterior
# samples clearly peak less than the data. The data show bigger values around x =
# 80 than the posteriors. I am not sure if it is kosher to say that the posteriors
# predict a wider distribution than the data indicate, nor am I sure if this is a
# problem. 

# Another not clear observation is why some values are seemingly below 0. Could it
# be because sometimes, the distance between Sem Different is lower than Sem
# Similar? I am really just guessing. 
```
